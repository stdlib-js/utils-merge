...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1Vl2V', options ) );
	}
	if ( hasOwnProp( options, 'level' ) ) {
		opts.level = options.level;
		if ( !isPositiveInteger( opts.level ) ) {
			return new TypeError( format( '1Vl3P', 'level', opts.level ) );
		}
	}
	if ( hasOwnProp( options, 'copy' ) ) {
		opts.copy = options.copy;
		if ( !isBoolean( opts.copy ) ) {
			return new TypeError( format( '1Vl2o', 'copy', opts.copy ) );
		}
	}
	if ( hasOwnProp( options, 'override' ) ) {
		...
		if (
			...
			return new TypeError( format( '1VlBW', 'override', opts.override ) );
		}
	}
	if ( hasOwnProp( options, 'extend' ) ) {
		opts.extend = options.extend;
		if ( !isBoolean( opts.extend ) ) {
			return new TypeError( format( '1Vl2o', 'extend', opts.extend ) );
		}
	}
	return null;
}
